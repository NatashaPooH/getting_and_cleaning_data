...
# downloads the Samsung data and extracts it
...
# The function loads and processes either a train or a test dataset,
# given that current directory is the Samsung dataset.
load.dataset <- function (set, features, labels) {
        ...
        setwd('..')
        write.csv(dataset, file = 'raw_dataset.csv', row.names = FALSE)
        write.csv(tidy.dataset, file = 'tidy_dataset.txt', row.names = FALSE, quote = FALSE)
